...
```{r}
# -----------------------------------------------
# VaR y ES por Alisado Exponencial (versión final)
# -----------------------------------------------

# alpha ya puede definirse aquí si no se declaró antes
alpha <- 0.95  # Puedes cambiar a 0.94 o 0.96 si lo deseas
...
for (j in 1:ncol(PL)) {
  ...
  VaR_tabla <- matrix(NA, nrow = length(dias), ncol = length(nc))
  ES_tabla  <- matrix(NA, nrow = length(dias), ncol = length(nc))
  
  for (d in seq_along(dias)) {
    VaR_tabla[d, ] <- resultado$VaR * sqrt(dias[d])
    ES_tabla[d, ]  <- resultado$ES * sqrt(dias[d])
  }
...
# Crear una tabla resumen con VaR a 1 día para todas las emisoras
resumen_VaR_1d <- data.frame()

for (nombre in names(VaR_Exp)) {
  fila <- VaR_Exp[[nombre]]$VaR[1, ]  # Fila 1: horizonte de 1 día
  fila_df <- data.frame(
    Emisora = nombre,
    `VaR_95%` = fila[1],
    `VaR_97%` = fila[2],
  ...
  resumen_VaR_1d <- rbind(resumen_VaR_1d, fila_df)
}
