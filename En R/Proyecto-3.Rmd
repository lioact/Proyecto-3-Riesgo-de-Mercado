...
En esta sección se calcula el VaR individual y el VaR del portafolio utilizando distintos métodos no paramétricos: Simulación Histórica, Simulación Monte Carlo bajo supuestos normales y de Laplace, Bootstrapping y Suavizamiento Exponencial. Estos métodos permiten estimar el riesgo de pérdida sin asumir una distribución específica de los rendimientos. Se consideran diferentes niveles de confianza (95%, 97% y 99%) y horizontes de tiempo (1, 7, 15, 30, 60, 90 y 180 días), así como diferentes tamaños de simulación. Adicionalmente, se calcula la Pérdida Esperada (ES) para cada método y nivel de confianza, con el fin de complementar la medición del riesgo extremo.
...
**Rendimientos**
Para el calculo de rendimientos diarios vamos a usar la siguiente formula para cada acción:
...
```{r}
knitr::kable(VaRIndD[["P1"]], caption = "VaR Individual - Emisora P1")
```
...
knitr::kable(ES.SH$VaR, caption = "Valor en Riesgo (VaR)", digits = 4)

```
```{r}
# Mostrar tabla de ES
knitr::kable(ES.SH$ES, caption = "Pérdida Esperada (Expected Shortfall, ES)", digits = 4)
```
...
Finalmente, se calcula el VaR individual y el VaR del portafolio utilizando métodos paramétricos, los cuales suponen una distribución específica de los rendimientos. Se incluyen las siguientes metodologías: VaR Paramétrico estándar, Delta-Normal, la expansión de Cornish-Fisher y el modelo EWMA (Exponentially Weighted Moving Average). Se mantiene la coherencia en niveles de confianza y horizontes de tiempo respecto a la sección anterior. Esta parte del análisis permite comparar los resultados obtenidos con los métodos no paramétricos y evaluar la sensibilidad del VaR ante diferentes supuestos distribucionales.
