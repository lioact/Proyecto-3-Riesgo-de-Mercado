...
## VaR individual y de portafolio por Simulación Monte Carlo Normal 
```{r, warning=FALSE}
...
for (s in numsim_vec) {
  for (j in seq_along(emisoras)) {
    mu <- mu_lap[j]
    b <- max(b_lap[j], 1e-6)  # asegura que b > 0
    precio_actual <- precio_actuales[j]
...
# === 4. Reformatear resultados en tabla deseada ===
tabla_final <- resultados %>%
  pivot_wider(
    names_from = Confianza,
    values_from = c(VaR, ES),
...
for (s in numsim_vec) {
  for (j in seq_along(emisoras)) {
    ...
    for (h in dias) {
     ...
     perdidas_extremas <- sim_perdidas_pt[sim_perdidas_pt > v]
